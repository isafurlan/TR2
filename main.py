...
# ---------------------------------------------------------------------
# Política 2 (Buffer-Based) e Política 3 (Híbrida estatística + jitter)
# ---------------------------------------------------------------------
# Mesma lógica já validada offline em policy_comparison.py (com os mesmos
# parâmetros), agora disponível também para rodar ao vivo contra o servidor
# real via --policy. A Política 1 acima não foi alterada.
...
def select_quality_buffer_based(representations, buffer_level_s):
...
def select_quality_policy3_hybrid(
    representations,
    ewma_throughput,
    std_throughput,
    jitter_ewma_ms,
...
def select_quality_dispatch(policy, representations, avg_throughput,
                             throughput_ewma_value, std_throughput,
                             jitter_ewma_ms, buffer_level_s):
    if policy == "1":
        return select_quality(representations, avg_throughput)
    elif policy == "2":
        return select_quality_buffer_based(representations, buffer_level_s)
    elif policy == "3":
        return select_quality_policy3_hybrid(
            representations,
...
def main():
    ...
    parser.add_argument("--policy", choices=["1", "2", "3"], default="1",
    ...
    if args.policy == "1":
        # Caminho padrão, idêntico ao comportamento original (sem --policy).
        metrics_logger = MetricsLogger()
        plots_dir = "relatorios/plots"
    else:
        # Politicas 2 e 3 sao opt-in: gravam em arquivos/pastas proprios para
        # nao sobrescrever os resultados da Politica 1 ja usados no relatorio.
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        metrics_logger = MetricsLogger(filename=f"metrics_policy{args.policy}_{timestamp}.csv")
        plots_dir = f"relatorios/plots/live_policy{args.policy}"
    ...
    for i in range(30):
        ...
        buffer_level_now = buffer_manager.get_buffer_level()
        quality = select_quality_dispatch(
            args.policy,
            representations,
            average_throughput(),
            throughput_ewma,
        ...
        buffer_before_download = buffer_level_now
    ...
    plotter = MetricsPlotter(metrics_logger, plots_dir=plots_dir)
