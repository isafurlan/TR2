...
class BufferManager:
    def __init__(self, segment_duration, min_play_buffer=2.0, target_buffer=15):
    ...
    def regulate_buffer(self):
        if self.buffer_level > self.target_buffer:
            sleep_time = min(self.buffer_level - self.target_buffer, self.segment_duration)
