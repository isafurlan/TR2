...
# ---------------------------------------------------------------------
# Política 3 — híbrida estatística com penalidade de jitter
# ---------------------------------------------------------------------
#
# Combina três ingredientes (todos sugeridos no enunciado):
#
#   1. EWMA da vazão: dá mais peso às amostras recentes do que uma média
#      móvel simples. É a mesma estrutura matemática usada pelo TCP para
#      estimar o RTT (SRTT = (1-alpha)*SRTT + alpha*amostra), o que dá um
#      embasamento teórico direto e fácil de defender.
#
#   2. Margem por desvio padrão: a estimativa "segura" de vazão não é só a
#      EWMA, e sim EWMA - k*desvio_padrao. Quando a rede está mais instável
#      (desvio padrão alto), a estimativa fica mais conservadora mesmo que
#      a média não tenha caído - o que o baseline (fator fixo de 0.85) não
#      enxerga.
#
#   3. Penalidade explícita de jitter: a estimativa segura é reduzida
#      proporcionalmente ao jitter EWMA medido (jitter_ewma_ms). Jitter alto
#      indica que o tempo de entrega dos segmentos está variando bastante;
#      mesmo com vazão média OK, a chance de um segmento específico demorar
#      muito mais que a média aumenta, o que ameaça o buffer (que continua
#      drenando em tempo real enquanto se espera o segmento). Por isso é
#      correto ser mais conservador quando o jitter sobe.
#
#   4. Ajuste consciente de buffer: se o buffer já tem bastante colchão, a
#      política relaxa um pouco a conservadorice acima, evitando repetir a
#      subutilização de banda que a Política 1 apresentou.
...
def simulate_policy(trace, policy_name, segment_duration_s, history_window=5):
    ...
    for sample in trace:
        ...
        elif policy_name == "policy3_hybrid":
            representation = select_policy3_hybrid(
                REPRESENTATIONS,
                ewma_throughput if ewma_throughput is not None else 0.0,
                rolling_std(throughput_history),
                sample["jitter_ewma_ms"],
                buffer_level_s,
            )
        else:
        ...
        ewma_throughput = update_ewma(ewma_throughput, throughput_kbps)
